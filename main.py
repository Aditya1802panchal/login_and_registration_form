def register():
    ...
    password = input("Create your password with atleast one capital letter one integer and one special character within 5-16 characer: ")
    s = False
...
def login():
    ...
    if X in d:
        ...
        Y=Y.strip()
        file1=open("data_file.txt","r").readlines()
        for x in file1:
            x=x.strip()
            info=x.split(",")
